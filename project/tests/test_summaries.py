...
def test_read_summary(test_app_with_db, monkeypatch):
    ...
    assert response_dict["id"] == int(summary_id)
    assert response_dict["url"] == "https://foo.bar"
    # assert response_dict["summrary"]
...
def test_read_all_summaries(test_app_with_db, monkeypatch):
    ...
    assert len(list(filter(lambda d: d["id"] == int(summary_id), response_list))) == 1
...
def test_update_summary(test_app_with_db, monkeypatch):
    ...
    assert response_dict["id"] == int(summary_id)
    assert response_dict["url"] == "https://foo.bar"
    assert response_dict["summrary"] == "updated!"
    assert response_dict["created_at"]
